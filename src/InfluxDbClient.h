...
class Point {
  public:
    ...
    void addField(String name, float value);
    ...
    bool hasFields() { return _fields.length() > 0; }
    // True if a point contains at least one tag
    bool hasTags() { return _tags.length() > 0; }
...
class InfluxDBClient {
 public:
   ...
   int getLastHTTPStatusCode() { return _lastHTTPcode; }
  protected:
    // Checks params and sets up security, if needed
    void init(const char *pstrCert);
  private:
    // Connection info
    String _serverUrl;
    String _bucket;
